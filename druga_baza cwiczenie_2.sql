...
Select tresc from pytania where kategoria = 'programowanie';
Select tresc from pytania where rok= 2010;
Select tresc from pytania where answer = 'a';
...
Select kategoria, count(*) from pytania group by kategoria;
Select count(*) as rok2012 from pytania where rok = 2012;
Select tresc from pytania where odpa like '%cin%' or odpb like '%cin%' or odpc like '%cin%' or odpd like '%cin%';
Select tresc from pytania where kategoria ='multimedia i grafika' and rok > 2008;
Select tresc from pytania where rok = (select min(rok) from pytania);
Select tresc from pytania where rok = (select max(rok) from pytania);
Select avg(rok) from pytania;
Select tresc from pytania where LENGTH(odpa) < 5 or LENGTH(odpb) < 5 or LENGTH(odpc) < 5 or LENGTH(odpd) < 5; 
Select tresc from pytania order by tresc asc limit 5;
Select tresc, kategoria from pytania where rok = 2006
order by kategoria asc;
Select rok, Count(*) from pytania group by rok;