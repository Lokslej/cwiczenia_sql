...
Select tresc from pytania where answer ='c';
Select tresc from pytania where kategoria = 'multimedia i grafika' and rok =2008;
Select tresc from pytania where tresc like "%komputer%";
Select tresc, rok from pytania order by rok asc;
Select tresc from pytania order by rok asc limit 5;
Select tresc, rok from pytania where kategoria = 'systemy operacyjne i sieci'
order by rok desc;
Select Count(*) from pytania;
Select rok, Count(*) as liczbapytan from pytania
group by rok
order by liczbapytan desc limit 1;
Select kategoria, Count(*) as liczbapytan from pytania
group by kategoria
order by liczbapytan desc;
Select avg(rok) as srednirok from pytania where kategoria ='programowanie';
Select tresc from pytania where rok = 2005;
Select tresc from pytania where answer = 'b';
Select tresc from pytania where kategoria = 'urzadzenia techniki komp.' and rok between 2010 and 2015;
Select tresc from pytania where rok < 2000;
Select tresc from pytania where tresc like "%siec%";
Select kategoria, tresc from pytania order by kategoria asc;
Select tresc, rok from pytania where answer = 'd'
order by rok desc;
Select tresc, rok from pytania order by rok desc limit 5;
Select tresc, rok from pytania where kategoria = 'programowanie';
Select kategoria, answer from pytania where rok > 2015;
Select tresc, kategoria from pytania where answer = 'c';
Select tresc, rok from pytania where rok = 2005 or rok = 2006 or rok = 2007;
Select tresc, kategoria from pytania where kategoria = 'programowanie' or kategoria = 'multimedia i grafika';
Select tresc, rok from pytania where not rok = 2010;
Select tresc, answer, kategoria from pytania where answer = 'a' and kategoria = 'systemy operacyjne i sieci';
Select tresc, kategoria, rok from pytania where not kategoria = 'urzadzenia techniki komp.' and rok > 2000;
Select tresc from pytania limit 3;
Select tresc, rok from pytania order by rok desc limit 10;
